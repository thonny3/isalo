...
export default function TableChambre() {
  const {chambre} =  useChambre()
  const [currentPage, setCurrentPage] = useState(1);
  const [pageGroup, setPageGroup] = useState(1);
  const itemsPerPage = 5;

  // Nombre total de pages
  const totalPages = Math.ceil(chambre.length / itemsPerPage);
  const pagesPerGroup = 3; // Nombre de pages visibles par groupe

  // Données pour la page actuelle
  const currentData = chambre.slice(
    ...
    <table className="min-w-full table-auto text-left">
        <thead>
            <tr className="bg-gray-100">
                ...
                <th className="px-4 py-2">Nombre de  lits  </th>
                <th className="px-4 py-2">Types de  lits  </th>
                <th className="px-4 py-2">Prix unité </th>
            ...
            {currentData.map((row, index) => (
                <tr key={index} className="border-b">
                    <td className="px-4 py-2 ">
                        <span className="ml-2">{row. numero_chambre}</span>
                    </td>
                    <td className="px-4 py-2 ">
                       {row. id_categorie_chambre}
                    </td>
                    <td className="px-4 py-2 ">
                       {row. nombre_lits}
                    </td>
